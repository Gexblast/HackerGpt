...
const WALL_PROXIMITY_ATR_MULT = 0.35; // "approaching the wall" band, scaled to recent volatility
...
async function computeWallSniperSignal(idx, tf) {
  ...
  // Volume spike on the wall strike (vs average volume across the chain)
  ...
  const resRow = atResistance ? wallRow(resistanceWall, 'CE') : null;
  const supRow = atSupport ? wallRow(supportWall, 'PE') : null;
  const resVolSpike = resRow && avgVol > 0 && resRow.volume > avgVol * 1.5;
  const supVolSpike = supRow && avgVol > 0 && supRow.volume > avgVol * 1.5;

  // Candle confirmation
  const bounceCandle = last.close > last.open && last.close > prev.close; // green, closing higher
  const rejectionCandle = last.close < last.open && last.close < prev.close; // red, closing lower
  ...
  if (atSupport && bounceCandle && supVolSpike) {
    verdict = 'BUY_CE'; side = 'CE'; conf = 65;
    reasons.push('✅ Bounce candle off the PUT WALL (support) + volume spike (' + Math.round(supRow.volume) + ' vs avg ' + Math.round(avgVol) + ') — textbook entry');
  ...
  } else if (atResistance && rejectionCandle && resVolSpike) {
    verdict = 'BUY_PE'; side = 'PE'; conf = 65;
    reasons.push('✅ Rejection candle off the CALL WALL (resistance) + volume spike (' + Math.round(resRow.volume) + ' vs avg ' + Math.round(avgVol) + ') — textbook entry');
    ...
    reasons.push('👀 At support wall — waiting for a bounce candle' + (!supVolSpike ? ' + volume to spike' : ' (volume already spiking)'));
  } else if (atResistance) {
    reasons.push('👀 At resistance wall — waiting for a rejection candle' + (!resVolSpike ? ' + volume to spike' : ' (volume already spiking)'));
  } else {
  ...
  return {
    ...
    pcr, walls: { resistance: resistanceWall, support: supportWall, band: +band.toFixed(2) },
    stale, generatedAt: new Date().toISOString(),
...
async function wallSniperAlertTick() {
  ...
  for (const idx of Object.keys(INDEXES)) {
    try {
      ...
      if (!out._pushKey || out.stale) continue;
      if (wallLastPush.get(out._pushKey) === out._pushTime) continue;
      wallLastPush.set(out._pushKey, out._pushTime);
      await broadcastPush({
        title: '🎯 ' + idx + ' ' + (out.side === 'CE' ? 'BUY CALL' : 'BUY PUT') + (out.pick ? ' ' + out.pick.strike + ' ' + out.pick.type : ''),
        body: (out.pick && out.pick.ltp != null ? '₹' + out.pick.ltp + ' · ' : '') + 'confidence ' + out.confidence + '/100' + (out.pick && out.pick.stopLoss ? ' · SL ₹' + out.pick.stopLoss + ' · book½ ₹' + out.pick.bookHalfAt : '') + '. Confirm before entering.'
